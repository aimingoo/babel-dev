...
class ExpressionParser extends LValParser {
  ...
  parseObjectMember(isPattern, refShorthandDefaultPos) {
    ...
    prop.accessibility = this.tsParseModifier(["public", "private"]);
    prop.method = false;
...
class StatementParser extends ExpressionParser {
  ...
  parseTrySpecialClassMemberName(classBody, member, isStatic, containsEsc) {
    let key = this.parseIdentifier(true);
  ...
  parseClassMember(classBody, member, state, constructorAllowsSuper) {
    ...
    if (this.match(types.name) && this.state.value == "internal") {
      if (this.parseTrySpecialClassMemberName(classBody, member, isStatic, containsEsc)) return;
    ...
    if (member.withInternal && isPublic) {
      this.raise(key.start, "Public member can not as internal.");
    }
    ...
    while (this.match(types.name)) {
      ...
      if (specialNames.indexOf(key) < 0) break;
      if (this.parseTrySpecialClassMemberName(classBody, member, isStatic, containsEsc)) return;

      if (key === "static") {
        isStatic = true;
      } else if (key === "as") {
        member.asFrom = "itself";
      }
    }
  ...
  parseClassMemberWithIsStatic(classBody, member, state, isStatic, constructorAllowsSuper) {
    ...
    const containsEsc = this.state.containsEsc;
    let key = this.parseClassPropertyName(member);

    if (this.eatContextual("as")) {
      member.asFrom = key;
      key = this.parseClassPropertyName(member);
    }
